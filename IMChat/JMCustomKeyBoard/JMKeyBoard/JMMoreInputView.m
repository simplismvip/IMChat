...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        NSArray *icons = @[@"image", @"camera", @"files", @"video", @"collect", @"money", @"location", @"call"];
        NSArray *names = @[@"相册", @"相机", @"文件", @"视频", @"收藏", @"钱包", @"位置", @"电话"];
        for (int i = 0; i < names.count; i ++) {
            
            JMButtom *btnAction = [JMButtom buttonWithType:(UIButtonTypeSystem)];
...
- (void)layoutSubviews
{
    ...
    for (UIView *view in self.subviews) {
        ...
        if ([view isKindOfClass:[JMButtom class]]) {
